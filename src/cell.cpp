...
cell::solve(const configuration &config) {
    ...
    cell_observer observer(solution_abundances, solution_times, store_every,
                           dump_every);
...
void cell::operator()(const abundance_v &x, abundance_v &dxdt, const double t) {
    ...
    for (auto i = 0; i < net->n_reactions; i++) {
        if (net->reactions[i].type == REACTION_TYPE_NUCLEATE) {
            ...
            auto pressure_equilibrium =
                equPres(net->reactions[i].alpha, net->reactions[i].beta, temperature);
            auto pressure = x[key_idx] * kBeta(temperature);
            auto saturation = pressure / pressure_equilibrium;

            auto critical_size =
                net->nucl_rate_data[net->reactions[i].id].interpolate(1, temperature,
                        saturation);
            if (critical_size > 0.0) {
                auto log_nucleation_rate =
                    net->nucl_rate_data[net->reactions[i].id].interpolate(0, temperature,
                            saturation);
